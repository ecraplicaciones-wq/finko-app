...
export function updateDash() {
  ...
  const filasMovimientos = S.gastos.slice(0, 6).map(g => {
    ...
    return `<tr>
      ...
      <td><div style="font-weight:600">${he(g.desc)}</div><div style="font-size:10px; color:var(--t3); margin-top:2px;">${CATS[g.cat] || '📦 Otro'}</div></td>
  ...
  setHtml('d-rec', S.gastos.length
    ? filasMovimientos.join('')
    : '<tr><td colspan="5" class="emp">Sin movimientos</td></tr>'
  );
